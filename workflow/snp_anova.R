...
for (i in 1:ncol(genotypes_batch)){
  ...
  anova_model <- aov(as.numeric(snp_data) ~ Cluster + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9, data = phenotype_data )
  anova_summary <- summary(anova_model)
  anova_f <- anova_summary[[1]][["F value"]][1]
  anova_pval <- anova_summary[[1]][["Pr(>F)"]][1]
  
  # 2. Trend test (ordinal encoding of cluster)
  trend_model <- lm(snp_data ~ numeric_cluster + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9, data = phenotype_data)
  ...
  pc1_model <- lm(snp_data ~ pc1_clinical + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9, data = phenotype_data)
